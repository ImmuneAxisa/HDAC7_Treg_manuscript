...
objects_dir <- "./results/seurat_objects/"
dir.create(objects_dir, showWarnings = FALSE)


results_dir <- "./results/clustering_typing"
...
merged.seurat <- readRDS(file.path(objects_dir,"master_seurat_post_harmony.rds"))
...
cluster_assignments <- merged.seurat@meta.data$SNNk20_algo1_res0.05
...
print("tSNE")
merged.seurat <- RunTSNE(merged.seurat, reduction = "pca", dims = 1:30)
...
saveRDS(reductions.meta, file.path(results_dir,"master_seurat_low_res_clustering_table_oriPCA.rds"))


saveRDS(merged.seurat, file.path(objects_dir,"master_seurat_harmony_low_res_oriPCA.rds"))
