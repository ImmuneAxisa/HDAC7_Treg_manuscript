...
# get seurat object
merged.seurat <- readRDS(file.path(objects_dir,"master_seurat_harmony_low_res_clustered.rds"))
...
presto_results <- wilcoxauc(merged.seurat, "Sub_res0.05", assay = 'data')

saveRDS(presto_results, file.path(results_dir,"presto_markers_Sub_res0.05.rds"))
...
cluster_assignments <- merged.seurat@meta.data$Sub_res0.05
...
paired_results <- lapply(combinations, function(comb) {
    wilcoxauc(merged.seurat, "Sub_res0.05", groups_use = comb, assay = 'data') %>%
        mutate(groupA = comb[1]) %>%
        mutate(groupB = comb[2])
})

saveRDS(paired_results, file.path(results_dir,"presto_paired_markers_Sub_res0.05.rds"))

paired_results <- do.call(rbind, paired_results) %>%
  dplyr::filter(padj < 0.01 & logFC > 0 & auc > 0.6)

saveRDS(paired_results, file.path(results_dir,"presto_paired_markers_Sub_res0.05_filtered.rds"))
