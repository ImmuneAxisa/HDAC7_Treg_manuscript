...
gseaplot2 <- function(x, geneSetID, title = "", color="green", base_size = 11,
  ...
  p <- ggplot(gsdata, aes_(x = ~x)) + xlab(NULL) +
    ...
    theme(panel.grid.major = element_line(colour = "grey92"),
          ...
          axis.title = element_text(size=base_size), #added by PP
          panel.border = element_rect(colour = "black", fill=NA, size=1)) +
    scale_x_continuous(expand=c(0,0))
  
  if (ES_geom == "line") {
    es_layer <- geom_line(aes_(y = ~runningScore, color= ~Description), size=0.5)
  } else {
  ...
  p.res <- p.res + ylab("Running Enrichment Score") +
    ...
    geom_hline(yintercept = 0, linetype = "dashed") ## added by PP
