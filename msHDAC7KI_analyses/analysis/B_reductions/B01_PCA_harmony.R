...
HVG <- merged.seurat@assays[["RNA"]]@meta.features %>%
                    ...
                    dplyr::filter(gene_category == "other" & vst.variance.standardized >1) %>% # removed  & vst.mean > 10^-3
                    pull(gene_name)
...
ggsave(file.path(results_dir,"master_seurat_PCA.pdf"), plot_grid(p1,p2,p3,p5,p6,p7,p8,ncol = 3), width = 16, height = 16)

saveRDS(reductions.meta, file.path(results_dir,"master_seurat_meta_table.rds"))
